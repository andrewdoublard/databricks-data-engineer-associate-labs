...
# MAGIC %md
# MAGIC ### NOTES BEFORE CONTINUING:
# MAGIC
# MAGIC - The JSON schema in the exercise differs slightly from the provided solution schema.
# MAGIC - The `event_timestamp` field is provided as a string in the source JSON. While it can be ingested as a TIMESTAMP in Delta tables, COPY INTO fails due to implicit casting during schema reconciliation.
# MAGIC - In production Databricks pipelines, this issue is typically avoided by using Auto Loader (cloudFiles), which handles schema inference and evolution more robustly than COPY INTO.
# MAGIC - COPY INTO is designed for idempotent file ingestion rather than strict schema enforcement or complex type handling.
# MAGIC - For this exercise, I have maintained an explicit schema to reinforce structured ingestion concepts of this exercise.
